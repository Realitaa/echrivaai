...
describe('Locale Switch', function () {
    ...
    it('switches locale correctly for guest users', function () {
        ...
        $response->assertRedirect()->assertCookie('locale', 'en');
    ...
    it(
        'overrides guest cookie locale using authenticated user locale',
        function () {
    ...
    it(
        'does not overwrite database locale when guest locale exists',
        function () {
            $user = User::factory()->create([
                'locale' => 'id',
            ]);

            $this->withCookie('locale', 'fr')->actingAs($user)->get('/');
    ...
    it(
        'updates database and cookie when authenticated user switches locale',
        function () {
            $user = User::factory()->create([
                'locale' => 'id',
            ]);
    ...
    it('uses locale from second authenticated account', function () {
        ...
        $this->actingAs($userA)->get('/')->assertOk();

        expect(app()->getLocale())->toBe('en');

        auth()->logout();

        $this->actingAs($userB)->get('/')->assertOk();

        expect(app()->getLocale())->toBe('id');
    });

    it(
        'second authenticated account overrides previous cookie locale',
        function () {
            $userA = User::factory()->create([
                'locale' => 'fr',
            ]);
            ...
            $responseA = $this->actingAs($userA)->patch(
                route('locale.update'),
                [
                    'locale' => 'fr',
                ],
            ...
            $responseB = $this->withCookie('locale', 'fr')
                ->actingAs($userB)
                ->get('/');
    ...
    it(
        'authenticated locale overrides browser locale after login',
        function () {
            $user = User::factory()->create([
                'locale' => 'en',
            ]);
    ...
    it(
        'changes runtime locale immediately after switching locale',
        function () {
            $user = User::factory()->create([
                'locale' => 'id',
            ]);

            $this->actingAs($user)->patch(route('locale.update'), [
                'locale' => 'fr',
            ]);
    ...
    it(
        'persists switched locale across multiple authenticated requests',
        function () {
            $user = User::factory()->create([
                'locale' => 'en',
            ]);
    ...
    it('persists switched locale across multiple guest requests', function () {
        $this->withCookie('locale', 'fr')->get('/')->assertOk();
