...
describe('Locale Security', function () {
    ...
    it('does not overwrite authenticated locale using cookie locale', function () {
        $user = User::factory()->create([
            'locale' => 'id',
        ]);
    ...
    it('ignores malformed browser locale header', function () {
        $response = $this
            ->withHeader('Accept-Language', '<script>alert(1)</script>')
            ->get('/');
    ...
    it('guest locale cannot override authenticated database locale', function () {
        $user = User::factory()->create([
            'locale' => 'fr',
        ]);
