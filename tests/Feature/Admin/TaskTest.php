...
test('admin can view task list table', function () {
    ...
    $this->actingAs($admin)
        ...
        ->assertSuccessful()
        ->assertInertia(fn(Assert $page) => $page->component('admin/Task'));
});

test('non-admin cannot access task list table', function () {
    $user = User::factory()->create(['role' => 'teacher']);

    $this->actingAs($user)->get(route('admin.task.index'))->assertForbidden();
});
...
test('admin can search task by title', function () {
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(
            fn(Assert $page) => $page->where('tasks.data', function ($tasks) {
...
test('empty search returns all tasks', function () {
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(fn(Assert $page) => $page->has('tasks.data', 3));
});
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(
            fn(Assert $page) => $page->where(
                'tasks.data',
                fn($tasks) => count($tasks) === 1 &&
                    $tasks[0]['title'] === 'Essay Final',
            ),
        );
});
...
test('pagination works properly', function () {
    ...
    $this->actingAs($admin)
        ->get(route('admin.task.index'))
        ->assertInertia(fn(Assert $page) => $page->has('tasks.data', 10));
});
