...
test('admin can view dashboard with stats', function () {
    ...
    User::factory()
        ->count(3)
        ->create(['role' => 'teacher', 'is_approved' => true]);
    User::factory()
        ->count(1)
        ->create(['role' => 'admin']);
...
test(
    'teacher can view dashboard with table of not approved teacher',
    function () {
        $admin = User::factory()->create(['role' => 'admin']);
        ...
        $response->assertInertia(
            fn(Assert $page) => $page
                ...
                ->has(
                    'notApprovedTeacher.0',
                    fn(Assert $page) => $page->where('id', $teacher->id)->etc(),
                ),
        );
    },
);
