...
const sidebarClassroomUrl = computed(() =>
    userRole === 'teacher' ? indexTeacher : indexStudent,
);
</script>
                    ...
                    <CollapsibleTrigger as-child>
                        <SidebarMenuButton
                            :tooltip="
                                userRole === 'teacher'
                                    ? $t(
                                          'navigation.toggle.teacherClassroomList',
                                      )
                                    : $t(
                                          'navigation.toggle.studentClassroomList',
                                      )
                            "
                            size="lg"
                        ...
                        >
                            ...
                            <ChevronUp
                                class="ml-auto size-5 transition-transform duration-300 group-data-[state=open]/collapsible:rotate-180"
                            />
                        </SidebarMenuButton>
                    </CollapsibleTrigger>
                        ...
                        <SidebarMenuSub class="mx-0 px-0">
                            <SidebarMenuSubItem v-if="classLists?.length === 0">
                                <SidebarMenuSubButton size="md" class="py-6">
                                    <span
                                        class="ml-1 text-sm text-muted-foreground italic"
                                        >{{
                                            $t('navigation.toggle.empty')
                                        }}</span
                                    >
                                </SidebarMenuSubButton>
                            </SidebarMenuSubItem>
                            <SidebarMenuSubItem
                                v-else
                                v-for="classroom in classLists"
                                :key="classroom.title"
                            >
                                <SidebarMenuSubButton
                                    as-child
                                    :is-active="
                                        isCurrentOrParentUrl(classroom.url)
                                    "
                                    size="md"
                                    class="py-6"
                                >
                                    <Link
                                        :href="
                                            sidebarClassroomUrl(classroom.id)
                                        "
                                    >
