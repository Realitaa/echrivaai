...
const statusConfig = (status: string) => {
    switch (status) {
        case 'processing':
            return {
                icon: Loader2,
                label: 'task.student.status.processing',
        ...
        case 'graded':
            return {
                icon: CheckCircle,
                label: 'task.student.status.graded',
        ...
        case 'failed':
            return {
                icon: AlertCircle,
                label: 'task.student.status.failed',
        ...
        default:
            return {
                icon: Send,
                label: 'task.student.status.submitted',
                variant: 'secondary' as const,
                color: 'text-gray-600',
                iconClass: '',
            };
...
<template>
    <Card
        ...
        :key="submission.id"
        class="shrink-0 cursor-pointer transition-all hover:border-primary/30 hover:shadow-md"
        :class="[
            currentView === 'detail' && selectedSubmissionId === submission.id
    ...
    >
        <CardContent class="flex items-center gap-4 py-4">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2">
                    <p class="text-sm font-semibold">
                        {{
                            $t('task.student.history.version', {
            ...
            <!-- Arrow -->
            <ArrowLeft
                class="h-4 w-4 shrink-0 rotate-180 text-muted-foreground"
                v-if="
                    currentView !== 'detail' ||
